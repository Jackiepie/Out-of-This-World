...
//planetInformation是Array, 把8个Dictionary存进planetInformation Array里，最后copy一份Array，然后返回一份复制的Array。
+ (NSArray *)allKnownPlanets
{
    NSMutableArray *planetInformation = [@[] mutableCopy];
    // "[[NSSting alloc] init]" equal to "@[]"
    // so "[@[] mutableCopy]" equal to "[[[NSString alloc] init] mutabaleCopy]"
